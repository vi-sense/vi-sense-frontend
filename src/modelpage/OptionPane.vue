<template>
  <div>
    ...
    <div>
      ...
      <v-subheader
        @click="setCamera('Rotation Camera')"
        style="cursor:pointer"
        :class="{active: activeCamera == 'Rotation Camera'}"
      >
        <v-icon :class="{active: activeCamera == 'Rotation Camera'}">mdi-rotate-3d-variant</v-icon>Rotation Camera
        <v-spacer />
      </v-subheader>

      <v-subheader
        @click="setCamera('Free Move Camera')"
        style="cursor:pointer"
        :class="{active: activeCamera == 'Free Move Camera'}"
      >
        <v-icon :class="{active: activeCamera == 'Free Move Camera'}">mdi-account</v-icon>Free Move Camera
        <v-spacer />
      </v-subheader>
    </div>
      ...
      <div v-for="clip of clippingPlanes" :key="clip.axis">
        ...
        <div class="flex-row">
          ...
          <input
            ...
            :min="clip.min"
            :max="clip.max"
            v-on:input="handleClippingPlane(clip.enabled, clip.axis, clip.value, clip.flipped)"
          />
          <v-tooltip bottom>
            <template #activator="{ on, attrs }">
              <v-icon
                @click="handleClippingPlane(clip.enabled, clip.axis, -clip.value, !clip.flipped)"
                color="rgba(82, 186, 162, 1)"
                v-bind="attrs"
                v-on="on"
                :disabled="!clip.enabled"
...
import {
  changeFOV,
  changeCameraClipping,
  switchCamera
} from "./babylon/cameras";
...
export default {
  ...
  methods: {
    OnBtnClick: function(event) {
      if (event) {
        console.log("WTF");
        eventBus.$emit("overlay-changed", true)
      }
    },
    setCamera(camera) {
      if (camera != this.activeCamera) switchCamera();
    },
